        ...
        function renderAgenda() {
            ...
            agenda.forEach((slot, index) => {
                ...
                const isCurrent = now >= new Date(slot.start).getTime() && now <= new Date(slot.end).getTime();
                const isDone = task && task.completed;

                const el = document.createElement('div');
                el.className = `agenda-slot ${isCurrent ? 'current' : ''} ${isDone ? 'done' : ''}`;
                ...
                <div class="agenda-info">
                    ...
                    <div class="agenda-actions">
                        ${(!isDeleted && !task.phantom) ? `<button class="btn" style="padding:2px 5px; font-size:10px" onclick="jumpToTask('${slot.taskId}')">🎯 View</button>` : ''}
        ...
        function clearCompletedAgendaSlots() {
            ...
            agenda = agenda.filter(slot => {
                ...
                // Keep if task not found (deleted) or task not completed
        ...
        function addQuickSlot() {
            let tid = selectedNodeId;
            // Fallback 1: selectedIds
            if (!tid && selectedIds.size > 0) tid = Array.from(selectedIds)[0];
            // Fallback 2: Focus HUD (Current Task)
            if (!tid) {
                const hud = document.getElementById('focus-hud');
                if (hud) tid = hud.dataset.doingTaskId || hud.dataset.shouldTaskId || hud.dataset.upcomingTaskId;
            }

            if (!tid) { alert("Select a task first or refer to the Focus HUD."); return; }
            const now = new Date();
            agenda.push({
                taskId: tid,
