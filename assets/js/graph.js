        ...
        function autoArrangeGraph() {
            ...
            const COL_GAP = 280;           // Standard horizontal gap
            const ROW_GAP = 180;           // Standard vertical gap
            const ACTIVE_START_X = 400;    // Standard alignment for first active nodes
            const TIME_SCALE = 15;         // Subtle timeline expansion (px per day)

            const processedIds = new Set();
            const components = [];

            // Helper: Breadth-First Search to collect all nodes in a weakly connected component
            function collectComponent(startNodeId) {
                const comp = [];
                const queue = [startNodeId];
                const visitedLocal = new Set([startNodeId]);
                ...
                while (queue.length > 0) {
                    ...
                    node.dependencies.forEach(d => {
                        if (!visitedLocal.has(d.id)) {
                            visitedLocal.add(d.id);
                            queue.push(d.id);
                        }
                    });
                    ...
                    // Add Children
                    nodes.forEach(other => {
                        if (other.dependencies.some(d => d.id === id) && !visitedLocal.has(other.id)) {
                            visitedLocal.add(other.id);
                            queue.push(other.id);
                        }
                    });
                }
            ...
            nodes.forEach(n => {
                if (!processedIds.has(n.id)) {
                    const comp = collectComponent(n.id);
                    components.push(comp);
                    comp.forEach(cn => processedIds.add(cn.id));
                }
            });

            // 3. Sort Components (Priority)
            components.sort((a, b) => {
                const getUrgency = (c) => Math.max(...c.map(n => (n.isManualUrgent ? 10 : (n.isAutoUrgent ? 5 : 0))));
                const getMinDate = (c) => Math.min(...c.map(n => (n.dueDate ? new Date(n.dueDate).getTime() : Infinity)));

                const urA = getUrgency(a);
                const urB = getUrgency(b);
                if (urA !== urB) return urB - urA; // Urgent components first

                const dateA = getMinDate(a);
                const dateB = getMinDate(b);
                if (dateA !== dateB) return dateA - dateB; // Then earliest due date

                return b.length - a.length; // Then largest components
            });
            ...
            components.forEach(comp => {
                ...
                // Logical Rank: Depth relative to the first ACTIVE node in a chain
                function getLogicalRank(nodeId) {
                    if (memoRank.has(nodeId)) return memoRank.get(nodeId);

                    const node = comp.find(n => n.id === nodeId);
                    if (!node) return 0;

                    // Active parents are those which are NOT completed
                    const activeParents = node.dependencies
                        .filter(d => d.type === 'hard')
                        .map(d => comp.find(p => p.id === d.id))
                        .filter(p => p && !p.completed);

                    if (node.completed || activeParents.length === 0) {
                        memoRank.set(nodeId, 0);
                        return 0;
                    }

                    let maxP = -1;
                    activeParents.forEach(p => {
                        maxP = Math.max(maxP, getLogicalRank(p.id));
                    });
                    const rank = maxP + 1;
                    memoRank.set(nodeId, rank);
                    return rank;
                }

                // Initial ranking
                comp.forEach(n => getLogicalRank(n.id));

                // Final Rank Assignment (handles completed tasks specifically)
                comp.forEach(n => {
                    let rank = memoRank.get(n.id);
                    if (n.completed) {
                        const hasActiveSuccessor = comp.some(other =>
                            !other.completed &&
                            other.dependencies.some(d => d.id === n.id)
                        );
                        // Column -1 for completed predecessors, -2 for terminal ones
                        if (hasActiveSuccessor) rank = -1;
                        else rank = -2;
                    }
                    nodeRanks.set(n.id, rank);
                });

                // Group by final ranks
                const rankGroups = new Map();
                comp.forEach(n => {
                    const r = nodeRanks.get(n.id);
                    if (!rankGroups.has(r)) rankGroups.set(r, []);
                    rankGroups.get(r).push(n);
                });

                // Vertical Sorting
                rankGroups.forEach(list => {
                    list.sort((a, b) => {
                        if (a.isManualUrgent && !b.isManualUrgent) return -1;
                        if (!a.isManualUrgent && b.isManualUrgent) return 1;
                ...
                // FIXED: Baseline date only considers INCOMPLETE tasks to prevent "Historical Drift"
                const activeNodesWithDates = comp.filter(n => !n.completed && n.dueDate);
                const minDue = activeNodesWithDates.length > 0
                    ? Math.min(...activeNodesWithDates.map(n => new Date(n.dueDate).getTime()))
                    : NaN;

                // Assign positions
                let maxNodesInCompRow = 0;
                ...
                sortedRanks.forEach(rank => {
                    ...
                    list.forEach((n, idx) => {
                        ...
                        if (rank >= 0 && n.dueDate && !isNaN(minDue)) {
                            const diff = new Date(n.dueDate).getTime() - minDue;
                ...
                currentY += (maxNodesInCompRow * ROW_GAP) + 120;
            });

            render();
            saveToStorage();
            fitView();
            showNotification("Logic-driven Smart Layout Applied");
        }
        ...
        function updateCalculations() {
            ...
            nodes.forEach(n => {
                ...
                if (n.completed || n.isManualNotUrgent) {
                    n._isCritical = false;
                    n._isUrgent = false;
                } else {
                    if (n._isManualUrgentPath) {
                        n._isUrgent = true;
                        n._isCritical = true;
                    } else if (slack < 0) {
                        n._isUrgent = true;
        ...
        function getTotalTime(node) {
            ...
            // 1. Sum up all completed sessions, ensuring they are treated as Numbers
            let total = (node.timeLogs || []).reduce((acc, log) => {
                return acc + (Number(log.duration) || 0);
            }, 0);

            // 2. Add current session if running, ensuring Number types
            if (node.activeTimerStart) {
                const start = Number(node.activeTimerStart);
                const now = Date.now();
                // Prevent negative time if system clocks are slightly out of sync
                if (now > start) {
                    total += (now - start);
                }
            }
