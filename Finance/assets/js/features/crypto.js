        ...
        const CRYPTO_SELL_PROCEEDS_CATEGORY = 'Crypto Sales';
        const CRYPTO_SELL_PROCEEDS_TX_PREFIX = 'crypto_sell_proceeds_';
        const CRYPTO_SELL_PROCEEDS_CURRENCIES = new Set(['PHP', 'USD', 'JPY']);

        function getCryptoBuyExpenseTransactionId(cryptoEntryId) {
            return `${CRYPTO_BUY_EXPENSE_TX_PREFIX}${String(cryptoEntryId || '').trim()}`;
        }

        function getCryptoSellProceedsTransactionId(cryptoEntryId) {
            return `${CRYPTO_SELL_PROCEEDS_TX_PREFIX}${String(cryptoEntryId || '').trim()}`;
        }
        ...
        function isAutoCryptoSellProceedsTx(tx) {
            const txId = String(tx?.id || '').trim();
            if (txId.startsWith(CRYPTO_SELL_PROCEEDS_TX_PREFIX)) return true;

            const source = String(tx?.autoGeneratedSource || '').trim().toLowerCase();
            if (source === 'crypto_sell') return true;

            const linkedCryptoId = String(tx?.linkedCryptoTransactionId || '').trim();
            const linkedType = String(tx?.linkedCryptoTransactionType || '').trim().toLowerCase();
            return !!linkedCryptoId && linkedType === 'sell';
        }

        function isCryptoSellFiatCashProceeds(cryptoTx) {
            if (!cryptoTx || cryptoTx.type !== 'sell') return false;
            const currency = String(cryptoTx.currency || '').trim().toUpperCase();
            return CRYPTO_SELL_PROCEEDS_CURRENCIES.has(currency);
        }
        ...
        function buildCryptoSellProceedsNotes(cryptoTx) {
            const symbol = String(cryptoTx?.symbol || cryptoTx?.tokenId || '').trim().toUpperCase();
            const amount = Number(cryptoTx?.amount || 0);
            const price = Number(cryptoTx?.price || 0);
            const currency = String(cryptoTx?.currency || 'PHP').trim().toUpperCase();
            ...
            const lines = ['Auto-created from crypto sell.'];
            if (Number.isFinite(amount) && amount > 0) {
                let summary = `Quantity: ${amount} ${symbol || 'tokens'}`;
                if (Number.isFinite(price) && price > 0) {
                    summary += ` @ ${formatCurrency(price, currency)}/token`;
        ...
        function buildCryptoSellProceedsPayload(cryptoEntryId, cryptoTx) {
            const entryId = String(cryptoEntryId || '').trim();
            const symbol = String(cryptoTx?.symbol || cryptoTx?.tokenId || 'Crypto').trim().toUpperCase() || 'CRYPTO';
            const supportedCurrency = CRYPTO_SELL_PROCEEDS_CURRENCIES.has(String(cryptoTx?.currency || '').trim().toUpperCase())
                ? String(cryptoTx.currency).trim().toUpperCase()
                : 'PHP';
            const phpTotal = Math.max(0, Number(cryptoTx?.phpTotal ?? cryptoTx?.total ?? 0) || 0);
            const rawOriginalProceeds = Number(cryptoTx?.amount || 0) * Number(cryptoTx?.price || 0);
            const originalProceeds = Number.isFinite(rawOriginalProceeds) && rawOriginalProceeds > 0
                ? rawOriginalProceeds
                : (supportedCurrency === 'PHP'
                    ? phpTotal
                    : convertToDisplayCurrency(phpTotal, 'PHP', supportedCurrency));
            const proceedsTxId = getCryptoSellProceedsTransactionId(entryId);

            return {
                desc: `Crypto Sale: ${symbol}`,
                merchant: String(cryptoTx?.exchange || '').trim() || null,
                tags: ['crypto', 'sale'],
                amt: phpTotal,
                originalAmt: Math.max(0, Number(originalProceeds || 0)),
                originalCurrency: supportedCurrency,
                quantity: 1,
                ...
                creditCardName: null,
                date: cryptoTx?.date || new Date().toISOString(),
                importId: `crypto_sell:${entryId}`,
                dedupeHash: proceedsTxId,
                linkedCryptoTransactionId: entryId,
        ...
        function cryptoSellProceedsPayloadMatches(existingPayload, nextPayload) {
            if (!existingPayload || !nextPayload) return false;
            return Object.keys(nextPayload).every(key => {
                return JSON.stringify(existingPayload[key] ?? null) === JSON.stringify(nextPayload[key] ?? null);
            });
        }
        ...
        async function syncCryptoSellProceedsInDB(db) {
            if (!db || typeof db !== 'object') {
                return { db, changed: false, added: 0, restored: 0, updated: 0, removed: 0 };
            }
            ...
            for (const cryptoEntry of db.crypto) {
                ...
                try {
                    ...
                    console.error('Failed to decrypt crypto transaction for sell proceeds sync.', error);
                    continue;
                }

                if (!isCryptoSellFiatCashProceeds(cryptoTx)) continue;

                const cryptoEntryId = String(cryptoEntry.id || '').trim();
                if (!cryptoEntryId) continue;

                activeSellIds.add(cryptoEntryId);

                const proceedsTxId = getCryptoSellProceedsTransactionId(cryptoEntryId);
                const existingIndex = existingTxIndexById.get(proceedsTxId);
                const existingEntry = typeof existingIndex === 'number' ? db.transactions[existingIndex] : null;
                const nextPayload = buildCryptoSellProceedsPayload(cryptoEntryId, cryptoTx);
                const nowTs = Date.now();

                if (existingEntry && !existingEntry.deletedAt) {
                    try {
                        const existingPayload = await decryptData(existingEntry.data);
                        if (cryptoSellProceedsPayloadMatches(existingPayload, nextPayload)) {
                            continue;
                        }
                    } catch (error) {
                        console.warn('Failed to decrypt existing crypto sell proceeds transaction; refreshing it.', error);
                    }
                }

                const encryptedProceeds = await encryptData(nextPayload);
                const nextEntry = {
                    id: proceedsTxId,
                    data: encryptedProceeds,
                    createdAt: Number.isFinite(Number(existingEntry?.createdAt))
                        ? Number(existingEntry.createdAt)
                        : (Number.isFinite(Number(cryptoEntry.createdAt)) ? Number(cryptoEntry.createdAt) : nowTs),
                    lastModified: nowTs,
                ...
                if (existingEntry && typeof existingIndex === 'number') {
                    ...
                    if (existingEntry.deletedAt) restored += 1;
                    else updated += 1;
                } else {
                    db.transactions.push(nextEntry);
                    existingTxIndexById.set(proceedsTxId, db.transactions.length - 1);
                    added += 1;
                }
            ...
            db.transactions = db.transactions.map(item => {
                const txId = String(item?.id || '').trim();
                if (!txId.startsWith(CRYPTO_SELL_PROCEEDS_TX_PREFIX)) return item;

                const linkedCryptoId = txId.slice(CRYPTO_SELL_PROCEEDS_TX_PREFIX.length);
                if (activeSellIds.has(linkedCryptoId) || item.deletedAt) return item;
            ...
            return { db, changed, added, restored, updated, removed };
        }

        async function syncCryptoMirrorTransactionsInDB(db) {
            const buyResult = await syncCryptoBuyExpensesInDB(db);
            const sellResult = await syncCryptoSellProceedsInDB(buyResult?.db || db);
            return {
                db: sellResult?.db || buyResult?.db || db,
                changed: !!(buyResult?.changed || sellResult?.changed),
                buy: buyResult,
                sell: sellResult
            };
        }
