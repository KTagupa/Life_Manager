        ...
        function setTType(t) {
            const normalizedType = t === 'non_income_cash_in' ? 'non_income_cash_in' : (t === 'income' ? 'income' : 'expense');
            document.getElementById('t-type').value = normalizedType;
            const btnExp = document.getElementById('btn-expense');
            const btnInc = document.getElementById('btn-income');
            const btnNonIncome = document.getElementById('btn-non-income-cash-in');
            ...
            if (modalPanel) {
                modalPanel.dataset.transactionTone = normalizedType === 'income'
                    ? 'income'
                    : (normalizedType === 'non_income_cash_in' ? 'non-income' : 'expense');
            }

            if (btnExp) btnExp.className = normalizedType === 'expense' ? `${baseButtonClass} is-active` : baseButtonClass;
            if (btnInc) btnInc.className = normalizedType === 'income' ? `${baseButtonClass} is-active` : baseButtonClass;
            if (btnNonIncome) btnNonIncome.className = normalizedType === 'non_income_cash_in' ? `${baseButtonClass} is-active` : baseButtonClass;

            const categorySelect = document.getElementById('t-category');
            const isEditing = !!String(document.getElementById('t-id')?.value || '').trim();
            if (categorySelect && !isEditing) {
                const values = Array.from(categorySelect.options).map(option => option.value);
                const currentCategory = categorySelect.value;
                const typeDefaultCategories = new Set(['Food', 'Salary', 'Non-Income Cash In', 'Refund/Reimbursement', 'Asset Sale', 'Own Transfer', 'Others']);
                if (normalizedType === 'non_income_cash_in' && typeDefaultCategories.has(currentCategory) && values.includes('Non-Income Cash In')) {
                    categorySelect.value = 'Non-Income Cash In';
                } else if (normalizedType === 'income' && typeDefaultCategories.has(currentCategory) && values.includes('Salary')) {
                    categorySelect.value = 'Salary';
                } else if (normalizedType === 'expense' && typeDefaultCategories.has(currentCategory) && values.includes('Food')) {
        ...
        function openTransactionModal(id = null, options = {}) {
            ...
            if (id) {
                ...
                const isAutoCryptoCashIn = item.type === 'crypto_sell_proceeds'
                    || String(item.autoGeneratedSource || '').trim().toLowerCase() === 'crypto_sell'
                    || String(item.linkedCryptoTransactionId || '').trim();
                if (isAutoCryptoCashIn) {
                    alert("This cash-in was auto-created from crypto activity. Please edit the source crypto transaction from the crypto portfolio instead.");
                    return;
                }
        ...
        async function saveTransaction() {
            ...
            const finalType = type === 'income' && linkedLentId ? 'non_income_cash_in' : type;
